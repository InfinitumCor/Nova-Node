...
Nova is headless by design. If you want to build her a face, everything a UI needs is broadcast on the local WebSocket bridge (`ws://127.0.0.1:8765`) — states, transcripts, typed input. See `nova_websocket.py` for the protocol. (The legacy `electron/` prototype was removed in v4 — it called the API from the browser, which is unsafe.)
