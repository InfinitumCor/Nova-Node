...
def _save(data: dict):
    """Save to the persistent memory file — atomically. Write to a temp
    file in the same directory, then os.replace() over the target, so a
    crash mid-write can never truncate the only copy of her memory."""
