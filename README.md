...
(Porcupine    (local Whisper)    (Anthropic API)   (ElevenLabs / Edge-TTS)
 or STUB)
```
...
cp .env.example .env
# Edit .env — minimum required: ANTHROPIC_API_KEY, ELEVENLABS_API_KEY, ELEVENLABS_VOICE_ID
...
| **ElevenLabs** | High-quality voice (free tier available) | https://elevenlabs.io |

If ElevenLabs isn't configured, Nova falls back to the free Microsoft Edge-TTS voices automatically.
...
ElevenLabs voices each have an ID. To list yours:
